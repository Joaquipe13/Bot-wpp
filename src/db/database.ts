...
class DatabaseManager {
	...
	private constructor(pool: Pool, retryDelay: number = 2000, initDB: boolean = DB_INIT === 'true') {
		this.pool = pool;
	...
	public static async getInstance(): Promise<DatabaseManager> {
		...
		try {
			...
			while (retries > 0 && !connected) {
				try {
					const tempConn = await this.instance.pool.getConnection();
					await tempConn.ping();
					tempConn.release();
					connected = true;
				} catch (err: any) {
					...
					await new Promise((res) => setTimeout(res, this.instance.retryDelay));
					retries--;
					} else {
					throw err;
					}
			...
			if (!connected) throw new Error("❌ No se pudo conectar a la base de datos. Intentalo de nuevo en unos segundos.");
		} catch (err) {
			console.error("⚠️ Conexión a DB perdida. Railway probablemente la puso a dormir.");
